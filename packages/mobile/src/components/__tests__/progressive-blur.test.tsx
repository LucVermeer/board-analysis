...
describe('ProgressiveBlur', () => {
  ...
  it('uses the dark blur tint when the resolved scheme is dark', () => {
    themeMock.colorScheme = 'dark';
    const { container } = render(<ProgressiveBlur />);
    expect(container.querySelector('[data-testid="blur-view"]')?.getAttribute('data-blur-type')).toBe('dark');
  });

  it('uses the light blur tint when the resolved scheme is light (honours the in-app override)', () => {
    themeMock.colorScheme = 'light';
    const { container } = render(<ProgressiveBlur />);
    expect(container.querySelector('[data-testid="blur-view"]')?.getAttribute('data-blur-type')).toBe('light');
  });
});
