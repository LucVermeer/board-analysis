...
vi.mock('../../../providers/theme-provider', () => ({ useTheme: () => ({ systemColors: { tertiaryLabel: '#999' } }) }));
...
describe('SessionGradeStrip', () => {
  ...
  it('renders one grade-coloured bar per grade with height proportional to count (6px floor, 20px max)', () => {
    state.bars = bars([
      ['V3', 2, 'color-V3'],
      ['V5', 4, 'color-V5'],
      ['V6', 1, 'color-V6'],
    ...
    // maxCount = 4 → V5 = 20; V3 = round(2/4*20)=10; V6 = max(6, round(1/4*20)=5)=6 (floor).
    expect(els.map((node) => node.getAttribute('data-h'))).toEqual(['10', '20', '6']);
