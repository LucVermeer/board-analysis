...
export function ClimbSearchControls({
  ...
  // A single accent: the tinted background carries the grade colour; the border
  // stays a neutral hairline (no extra dot + coloured ring shouting the grade).
  const pillBackground = gradeActive ? (tintFromHex(accentHex) ?? systemColors.fill) : systemColors.fill;
  const pillBorder = systemColors.separator;
  ...
  return (
    <View style={styles.row}>
      <Pressable
        ...
        style={[styles.gradePill, { backgroundColor: pillBackground as string, borderColor: pillBorder as string }]}
      >
