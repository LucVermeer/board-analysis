...
export default function CreateClimbForm({
  ...
  const syncSavedClimbToQueue = useCallback(
    async (uuid: string, frames: string) => {
      ...
      const newItem = await queueActions.setCurrentClimb(climb, { clearPlaylistSuggestionSource: true });
      if (newItem) {
  ...
  const handleLoadDraft = useCallback(
    (climb: Climb) => {
      ...
      setQueueItemUuid(null);
      if (queueActions) {
        void queueActions.setCurrentClimb(climb, { clearPlaylistSuggestionSource: true }).then((newItem) => {
