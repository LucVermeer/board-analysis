...
// ---------------------------------------------------------------------------
// Quality-blend backfill migrations (0159 init + 0160 blend) replayed against a
...
// proves the 0159 guard makes a double-apply safe.
// ---------------------------------------------------------------------------

const drizzleDir = fileURLToPath(new URL('../../../db/drizzle/', import.meta.url));
const MIGRATION_0159 = readFileSync(`${drizzleDir}0159_init_upstream_quality_average.sql`, 'utf8');
const MIGRATION_0160 = readFileSync(`${drizzleDir}0160_backfill_quality_blend.sql`, 'utf8');

describe('quality-blend backfill (0159 + 0160) — real DB replay', () => {
  ...
  beforeEach(async () => {
    ...
    // Fresh guard slate so each replay starts un-applied (0159 recreates it).
  ...
  async function seedStats(uuid: string, opts: { upstream: number; qualityAverage: number | null }) {
    ...
    // upstream_quality_average / boardsesh_* are all NULL (0158 just added them).
  ...
  async function applyMigrations() {
    await client.unsafe(MIGRATION_0159);
    await client.unsafe(MIGRATION_0160);
  }
  ...
  async function seedEveryBranch() {
    ...
    // K2 — non-owned, upstream-rated, only an IMPORTED rated tick → NOT a 0160
    ...
    // STALE quality_average to prove 0160 recomputes the owned average.
    ...
    // K6 — non-owned, upstream-rated, NO ticks → 0159 seeds upstream_quality_average,
    // 0160 leaves it untouched (not a native-rated key).
    await seedClimb('K6', null);
    await seedStats('K6', { upstream: 8, qualityAverage: 4.5 });
  }

  it('produces the correct blend for every branch after 0159 + 0160', async () => {
  ...
  it('is double-apply safe — the 0159 guard prevents re-seeding a blended value', async () => {
    await seedEveryBranch();
    await applyMigrations();
    ...
    // Re-run BOTH migrations. Without the guard, 0159 would copy K1's now-blended
    ...
    const guard = (await db.execute(
      sql`SELECT count(*)::int AS n FROM _bs_migration_guards WHERE tag='0159_init_upstream_quality_average'`,
    )) as unknown as Array<{
      n: number;
    }>;
