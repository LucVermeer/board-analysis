...
4. **SEO content lives in the drawer's SSR output.** The drawer is keep-mounted today (`SwipeableDrawer keepMounted`). On the view route we mount it with `open={true}` so its server-rendered HTML carries the climb name, grade, setter, info sections. Crawlers see the same first-render content they see today; visually the drawer just animates in for users.
