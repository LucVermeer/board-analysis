...
- **Always-visible prev/next on the Queue Control Bar and in the Live Activity widget:** these render for everyone. They advance the wall along the queue → suggestions path and broadcast — **without** transferring leadership. The same handler runs for both driver and non-driver presses; the only conditional is that the existing leader-only auth check in `sessions/mutations.ts:408-409` must be relaxed for *this specific advance mutation* (or the mutation goes through a new "advance wall" path that allows any session participant). Leadership transfer is the lightbulb's job, not prev/next's. **If the session has no current leader, both buttons render disabled (greyed) with a short hint pointing at the lightbulb.** Server-side: the advance mutation must reject when there is no leader, so the disabled UI is enforced rather than advisory. The Live Activity widget surface needs the same disabled state (or its prev/next no-op'd) on the iOS side.
...
- Cover: solo BLE quickstart → tap → press lightbulb path; solo no-BLE state; party join → browse without consequence → take control via lightbulb → driver's drawer prev/next appear → released-and-yanked flows; non-driver pressing bar prev/next advances the wall but leader stays unchanged; non-driver pressing Live Activity prev/next advances the wall but leader stays unchanged; offline-and-back-online in a party; BLE-holder drop with 5s grace and claim-wall handoff; tapping the bar body opens the wall-view drawer (no prev/next, no swipe, "Currently on the wall" header, driver avatar inline, lightbulb still works).
