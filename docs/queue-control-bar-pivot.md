...
- _Non-driver, bar buttons:_ instant. Advances from the current wall climb (queue → suggestions fall-through), broadcasts. _Driver does not change_ — bar prev/next is shared queue-navigation, not a driver-transfer.
...
3. _Upcoming queue_ — items added via Add to Queue that haven't been broadcast yet, followed by suggested-climbs once the queue is exhausted (same fall-through as navigation).
...
- _New event `WallConfirmedClimb`._ Schema: `{ climbUuid: ID!, confirmedAt: DateTime!, confirmedByParticipantId: ID! }`. When any phone successfully delivers a climb to the board over BLE, that phone calls a `confirmWallClimb` mutation; the backend broadcasts the event to every member of the session over the existing party WS subscription. Each drawer that has an in-flight lightbulb press matches on `climbUuid` to clear its pending state.
...
Files: `packages/shared-schema/src/schema/session.ts`, `packages/shared-schema/src/types/events.ts`, `packages/backend/src/services/room-manager/room-manager.ts`, `packages/backend/src/graphql/resolvers/sessions/mutations.ts` (new `confirmWallClimb` and `setSessionBoardSerial` mutations), `packages/web/app/components/play-view/play-view-drawer.tsx` (the timer + fallback logic), `packages/web/app/components/board-bluetooth-control/bluetooth-context.tsx` (call `setSessionBoardSerial` after a successful connect; call `confirmWallClimb` after a successful frame send).
