...
6. **Bar and Live Activity prev/next advance the wall without transferring leadership.** Any party member can press them, driver or not. The wall advances along the shared queue (then falls through to suggestions); the change is broadcast to every member. The leader stays the leader — pressing prev/next here is "advance what we've agreed on", not "take over the session." This narrows the lightbulb's purpose: the lightbulb is the leadership-transfer gesture (commit a specific climb / claim the wall); the bar/Live Activity prev/next is the shared queue-navigation gesture that doesn't change who's driving. The drawer prev/next stays driver-only because the drawer is a browsing surface where the same gesture could be either intent — disambiguating on the wall-control surfaces (bar, widget) is enough.

   **If there is no current leader** (everyone disconnected, new joiner is alone, BLE-holder dropped past grace), bar and Live Activity prev/next are *disabled* (rendered greyed-out with a short hint — "Press the lightbulb to take the wall"). They never implicitly elevate anyone to leader. The lightbulb is the only path in. This matches the no-broadcast-without-an-author principle and makes the take-control gesture explicit at every entry point.
...
- **Non-driver, bar buttons or Live Activity buttons:** advance from the current wall climb (queue → suggestions fall-through), broadcast. **Leadership is NOT transferred** — the existing leader stays the leader. The bar/widget are shared wall-control surfaces; the lightbulb is the leadership-transfer gesture.
- **No-leader state, bar buttons or Live Activity buttons:** disabled. Greyed-out with a hint pointing the user at the lightbulb to take the wall. Never implicitly elevates anyone.
- **No-BLE state (nobody in the party has a Bluetooth connection), bar buttons or Live Activity buttons:** disabled. Greyed-out with a "Wall offline — connect a board" hint. The advance mutation rejects server-side; the iOS Live Activity widget must surface the rejection explicitly since it runs in a separate process. Lightbulb-press is still valid in this state (you can claim leadership pre-emptively, ready to drive once BLE comes up).
...
3. **Prev/next buttons in the normal drawer are hidden.** They only appear for the driver. The Queue Control Bar's prev/next *and* the Live Activity widget's prev/next remain visible to non-drivers — pressing either advances the wall through the shared queue (broadcast to everyone) but does *not* transfer leadership. The current leader stays the leader. To actually take over, press the lightbulb.
...
- **Always-visible prev/next on the Queue Control Bar and in the Live Activity widget:** these render for everyone. They advance the wall along the queue → suggestions path and broadcast — **without** transferring leadership. The same handler runs for both driver and non-driver presses; the only conditional is that the existing leader-only auth check in `sessions/mutations.ts:408-409` must be relaxed for *this specific advance mutation* (or the mutation goes through a new "advance wall" path that allows any session participant). Leadership transfer is the lightbulb's job, not prev/next's.
- **Advance mutation rejects in two states; lightbulb-press is always valid in both:**
  1. **No current leader.** Bar and Live Activity prev/next render disabled with a hint pointing at the lightbulb. Server rejects the mutation. Lightbulb press still works — it claims leadership.
  2. **No party member has an active BLE connection.** Bar and Live Activity prev/next render disabled with a "Wall offline — connect a board" hint. Server rejects the mutation (no physical pathway to broadcast). Lightbulb press still works — it lets you become leader pre-emptively so you're ready to drive once BLE is up. This state subsumes the BLE-holder-dropped-past-grace case from Phase 4; the same UI applies whether nobody ever connected BLE or the last BLE holder dropped.
- **iOS Live Activity widget must explicitly handle both rejection responses.** The widget runs in a separate process from the main app and won't pick up state changes silently. On receiving a reject from the advance mutation, the widget refreshes its content state to reflect the disabled/offline reason and shows a short error indication. See `packages/backend/src/handlers/widget-navigate.ts` for the existing widget-navigation handler — it needs the no-leader and no-BLE rejection paths added with distinct error codes so the widget can surface the correct hint.
...
- **Driver-first avatar group on the session mini bar.** The party member `AvatarGroup` (currently in `queue-control-bar.tsx:1025-1045` and the expanded variant at `:1112+`) sorts by `isLeader === true` first (the existing field on `SessionParticipant` / `SessionUser`). The driver's avatar carries a small lit-up lightbulb badge in the corner (overlay), matching the bar lightbulb's filled/glowing visual. Non-driver avatars render without a badge in their existing order. The expanded roster view follows the same ordering and badging rules so the driver is unambiguous wherever members are listed.
...
- Bar and Live Activity prev/next are disabled with a "Wall offline — connect a board" hint while in this state.
- **Server-side, the advance mutation rejects with a `NO_BLE` error code while in this state** (in addition to the `NO_LEADER` rejection from Phase 2). The iOS Live Activity widget handler at `packages/backend/src/handlers/widget-navigate.ts` propagates both reject codes back to the widget so it can render the correct hint.
- Lightbulb becomes a "Claim wall" affordance — pressing it makes you leader even without BLE, so you're ready to drive once BLE comes up. The lightbulb is never disabled by no-BLE; only the advance is.
...
- Cover: solo BLE quickstart → tap → press lightbulb path; solo no-BLE state; party join → browse without consequence → take control via lightbulb → driver's drawer prev/next appear → released-and-yanked flows; non-driver pressing bar prev/next advances the wall but leader stays unchanged; non-driver pressing Live Activity prev/next advances the wall but leader stays unchanged; offline-and-back-online in a party; BLE-holder drop with 5s grace and claim-wall handoff; no-leader state disables bar + Live Activity prev/next; no-BLE state (nobody in party has BLE, fresh session or post-drop) disables bar + Live Activity prev/next with "Wall offline" hint and rejects the advance mutation; pressing the lightbulb still works in no-BLE state and claims leadership pre-emptively; tapping the bar body opens the wall-view drawer (no prev/next, no swipe, "Currently on the wall" header, driver avatar inline, lightbulb still works).
...
- Session participant schema: `packages/shared-schema/src/schema/session.ts:147` (`SessionParticipant` type), `:175` (`participants` field on `Session`), `:18` and `:43` (`isLeader` fields — drop the "presentation/backward compatibility only" comment and document them as the driver indicator). `LeaderChanged` event type at `packages/shared-schema/src/types/events.ts:99`. Leader assignment + change logic + leader-only auth check at `packages/backend/src/graphql/resolvers/sessions/mutations.ts:280, 344, 408-409`.
- Open Graph session share image: `packages/web/app/api/og/session/route.tsx:34, 65` already uses `leaderName` for the share headline (`Join {name} on the wall`) — once driver = leader, that headline becomes accurate and load-bearing.
